...
def test_root(test_client):
    response = test_client.get("/v1/api//healthchecker")
    assert response.status_code == 200
    assert response.json() == {"message": "The API is LIVE!!"}


def test_create_get_user(test_client, user_payload):
    response = test_client.post("/v1/api//users/", json=user_payload)
    response_json = response.json()
    assert response.status_code == 201

    # Get the created user
    response = test_client.get(f"/v1/api//users/{user_payload['id']}")
    assert response.status_code == 200
    response_json = response.json()
...
def test_create_update_user(test_client, user_payload, user_payload_updated):
    ...
    response = test_client.patch(
        f"/v1/api//users/{user_payload['id']}", json=user_payload_updated
    )
    response_json = response.json()
...
def test_create_delete_user(test_client, user_payload):
    ...
    response = test_client.delete(f"/v1/api//users/{user_payload['id']}")
    response_json = response.json()
    assert response.status_code == 202
    assert response_json["Status"] == "Success"
...
def test_get_user_not_found(test_client, user_id):
    response = test_client.get(f"/v1/api//users/{user_id}")
    assert response.status_code == 404
    response_json = response.json()
...
def test_create_user_wrong_payload(test_client):
    response = test_client.post("/v1/api//users/", json={})
